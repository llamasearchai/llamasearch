...
class PDFProcessor:
    ...
    def __init__(self):
        ...
        if not self.ocrmypdf_available and not self.pytesseract_available:
            logger.warning(
                "Neither OCRmyPDF nor PyTesseract are available. OCR will be disabled."
            )
    ...
    def _check_ocrmypdf(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "OCRmyPDF not found. Install it with 'apt-get install ocrmypdf' or pip."
            )
            return False
    ...
    def _check_pytesseract(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "PyTesseract not found or Tesseract not installed. Install both for OCR functionality."
            )
            return False
    ...
    async def extract_text(
        ...
        if ocr_enabled and not (self.ocrmypdf_available or self.pytesseract_available):
            logger.warning(
                "OCR requested but OCR tools not available. Falling back to regular extraction."
            )
            ocr_enabled = False
    ...
    async def _process_pdf(
        self, pdf_path: str, ocr_enabled: bool, language: str
    ) -> str:
        """
        ...
        if ocr_enabled:
            ...
            if self.ocrmypdf_available:
                try:
                    ...
                    logger.error(
                        f"Error using OCRmyPDF: {str(e)}. Falling back to PyTesseract."
                    )
            ...
            if self.pytesseract_available:
                try:
                    ...
                    logger.error(
                        f"Error using PyTesseract: {str(e)}. Falling back to regular extraction."
                    )
    ...
    async def _extract_without_ocr(self, pdf_path: str) -> str:
        ...
        if not text.strip():
            logger.warning(
                f"No text extracted from {pdf_path}. The PDF might be scanned or contain only images."
            )
