...
def check_system_requirements():
    ...
    try:
        ...
        logger.warning(
            "git is not installed or not in PATH. Some features may not work properly."
        )
...
def check_linux_dependencies():
    ...
    try:
        ...
        logger.warning(
            "FFmpeg is not installed. Audio/video processing features will be limited."
        )
...
def check_macos_dependencies():
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "OCRmyPDF is not installed. PDF OCR features will be limited."
            )
        ...
        logger.warning(
            "Homebrew is not installed. It's recommended for installing dependencies on macOS."
        )
        logger.info(
...
def check_windows_dependencies():
    ...
    try:
        subprocess.run(
            ["choco", "--version"], check=True, capture_output=True, shell=True
        )
        ...
        try:
            subprocess.run(
                ["ffmpeg", "-version"], check=True, capture_output=True, shell=True
            )
            logger.info("FFmpeg is installed.")
        except (subprocess.CalledProcessError, FileNotFoundError):
            logger.warning(
        ...
        logger.warning(
            "Chocolatey is not installed. It's recommended for installing dependencies on Windows."
        )
        logger.info(
            "To install Chocolatey, follow the instructions at: https://chocolatey.org/install"
        )
...
def create_virtual_environment(force=False):
    ...
    if VENV_DIR.exists() and not force:
        logger.info(
            f"Virtual environment already exists at {VENV_DIR}. Use --force to recreate."
        )
        return
...
def setup_environment_vars():
    ...
    logger.info(
        "Please edit the environment file to set your API keys and other configuration options."
    )
...
def run_tests():
    ...
    try:
        subprocess.run(
            [str(python_path), "-m", "pytest", "-xvs"], check=True, cwd=PROJECT_ROOT
        )
        logger.info("All tests passed!")
    except subprocess.CalledProcessError as e:
        logger.error(f"Tests failed: {e}")
        logger.warning(
            "Installation may not be complete or some dependencies may be missing."
        )
        return False
...
def setup_gpu_environment():
    ...
    try:
        ...
        if cuda_available:
            ...
            logger.warning(
                "CUDA is not available for PyTorch. GPU acceleration will not be used."
            )

    except (subprocess.CalledProcessError, FileNotFoundError):
        logger.warning(
            "Failed to check CUDA availability. PyTorch may not be installed correctly."
        )
...
def check_mlx_environment():
    ...
    try:
        ...
        logger.warning(
            "MLX is not installed. Apple Silicon acceleration will not be used."
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Set up the development environment for Research Code Automation Tool."
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force recreation of virtual environment if it already exists",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpu",
        action="store_true",
        help="Set up GPU environment for machine learning tasks",
    )
    parser.add_argument(
        "--extras",
        nargs="+",
        help="Additional extras to install (e.g. ocr, scraping, academic)",
    )
    parser.add_argument("--skip-tests", action="store_true", help="Skip running tests")
    parser.add_argument(
        "--skip-hooks", action="store_true", help="Skip installing pre-commit hooks"
    )
