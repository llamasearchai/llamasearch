...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("llamasearch.log"), logging.StreamHandler()],
)
...
def load_env_file(env_file: str = ".env") -> None:
    ...
    if not os.path.exists(env_file):
        logger.warning(
            f"{env_file} file not found. Using existing environment variables."
        )
        return
...
def check_api_keys() -> Dict[str, bool]:
    ...
    for name, env_var in api_keys.items():
        ...
        if has_key:
            ...
            logger.warning(
                f"{name} API key not found. Set {env_var} to use {name} features."
            )
    ...
    try:
        ...
        try:
            ...
            config = uvicorn.Config(
                app=app, host=host, port=port, reload=True, log_level="info"
            )
            ...
            logger.error(
                "Make sure you're running this script from the LlamaSearchAI project directory"
            )
...
def start_monitoring():
    ...
    if os.environ.get("LLAMASEARCH_METRICS_ENABLED", "").lower() == "true":
        ...
        logger.info(
            "Metrics disabled. Set LLAMASEARCH_METRICS_ENABLED=true to enable monitoring."
        )
...
async def main() -> int:
    ...
    parser.add_argument(
        "--no-monitoring", action="store_true", help="Disable Prometheus monitoring"
    )
    ...
    if (
        ...
        and os.environ.get("LLAMASEARCH_METRICS_ENABLED", "").lower() == "true"
    ):
        start_monitoring()
