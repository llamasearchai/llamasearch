...
try:
    from llamafind.mlx_compat import is_mlx_available, should_use_mlx, log_mlx_status
...
def test_mlx_status() -> Dict[str, Any]:
    ...
    if status['mlx_available']:
        ...
        # Log compute units
        logger.info(f"MLX compute units: {status['config'].get('compute_units', 'unknown')}")
        ...
        for i in range(len(embedding_arrays)):
            for j in range(i+1, len(embedding_arrays)):
                ...
                if norm_i > 0 and norm_j > 0:
                    ...
                    similarity = np.dot(embedding_arrays[i], embedding_arrays[j]) / (norm_i * norm_j)
...
def test_text_generation(accelerator: Any) -> bool:
    ...
    if not accelerator or not hasattr(accelerator, "generate_text"):
        logger.error("MLX accelerator not available or doesn't support text generation.")
        return False
...
def test_ranking(accelerator: Any) -> bool:
    ...
    try:
        ...
        if not ranked_results or len(ranked_results) != len(results):
            logger.error(f"Ranking failed. Expected {len(results)} results, got {len(ranked_results) if ranked_results else 0}.")
            return False
        ...
        if hasattr(accelerator, "rerank_with_llm"):
            ...
            if not llm_ranked_results:
                ...
                logger.info(f"LLM reranking returned {len(llm_ranked_results)} results:")
...
def main():
    ...
    logger.info(f"MLX integration available: {test_results['mlx_integration_available']}")
