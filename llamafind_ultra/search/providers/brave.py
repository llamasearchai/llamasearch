...
class BraveSearch(SearchEngine):
    ...
    def __init__(self, api_key: Optional[str] = None, config: Optional[Dict[str, Any]] = None):
        ...
        self.api_base = self.config.get("api_base", "https://api.search.brave.com/res/v1")
    ...
    def search(self, query: Union[str, SearchQuery]) -> List[SearchResult]:
        ...
        try:
            ...
            params = {
                ...
                "count": search_query.num_results
            }
                ...
                if "age" in item:
                    try:
                        ...
                        if "minute" in age_str:
                            ...
                            published_date = datetime.now().replace(microsecond=0, second=0) - timedelta(minutes=minutes)
                        elif "hour" in age_str:
                            hours = int(age_str.split()[0])
                            published_date = datetime.now().replace(microsecond=0, second=0, minute=0) - timedelta(hours=hours)
                        elif "day" in age_str:
                            days = int(age_str.split()[0])
                            published_date = datetime.now().replace(microsecond=0, second=0, minute=0, hour=0) - timedelta(days=days)
