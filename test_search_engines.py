...
try:
    from llamafind.search_engines import get_engine, get_available_engines
    ...
    logger.error("Make sure you're running this script from the LlamaFind project directory")
    import sys
    sys.exit(1)
...
class EngineTestRunner:
    ...
    def __init__(self, engines: List[str] = None, use_mlx: bool = True, verbose: bool = False):
        """
        ...
        if engines:
            self.engines_to_test = [engine for engine in engines if engine in self.available_engines]
            if not self.engines_to_test:
                logger.warning(f"None of the specified engines {engines} are available. Using all available engines.")
    ...
    async def test_engine(self, engine_name: str, queries: List[str], num_results: int = 5):
        """
        ...
        for query in queries:
            try:
                ...
                logger.info(f"Found {len(results)} results for '{query}' in {elapsed_time:.2f}s")
    ...
    async def run_tests(self):
        ...
        for engine_name in self.engines_to_test:
            ...
            queries = test_queries.get(engine_name, ["python programming", "machine learning", "climate change"])
    ...
    def save_results(self):
        ...
        # Save to file
        filename = f"llamafind_engine_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def print_summary(self):
        ...
        for engine_name, queries in self.results.items():
            ...
            for query, data in queries.items():
                ...
                if data.get("sources"):
                    sources_str = ", ".join([f"{src}({count})" for src, count in data["sources"].items()])
